                ...
                if ($request->q) {
                    ...
                    if ($request->home_spm) {
                        ...
                        $source = HomeSlider::with('source_brands.brand')
                            ...
                            ->with(['source_categories.category' => function ($query) {

                                $query->leftJoin('product_categories as pc', function ($join) {
                                    $join->on('categories.id', '=', 'pc.category_id');
                                })->select('categories.*', 'pc.product_id as product_id');
            ...
            if (!is_null($source)) {
                ...
                if ((int)$source['source_type'] === Config::get('constants.sliderSourceType.CATEGORY')) {
                    ...
                    foreach ($source['source_categories'] as $item) {
                        array_push($itemIds, $item['category']['product_id']);
                    }
