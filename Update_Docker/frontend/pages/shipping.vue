...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /*  const singleShippingId = []

        if(Object.keys(this.cartShipping)?.length) {

          Object.keys(this.cartShipping).forEach(j => {
            const i = this.cartShipping[j]

            const sr = i?.shipping_place?.shipping_rule

            if(!sr?.single_price) {
              this.cartShipping[j] = {...this.cartShipping[j], single_shipping: true}

            } else if(sr?.single_price && (!singleShippingId[sr?.id] && sr?.id)) {

              this.cartShipping[j] = {...this.cartShipping[j], single_shipping: true}
              singleShippingId[sr?.id] = sr?.id

            } else {

              this.cartShipping[j] = {...this.cartShipping[j], single_shipping: false}
            }
          })
        }*/
      },
